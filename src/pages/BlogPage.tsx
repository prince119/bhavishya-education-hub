  ...
  <div className="min-h-screen">
    ...
    <div className="relative py-16" style={{ backgroundImage: `url(${headerImg})`, backgroundSize: "cover", backgroundPosition: "center" }}>
      ...
      <div className="container mx-auto px-4 text-center relative z-10">
        <h1 className="font-heading text-4xl font-bold text-secondary-foreground">Blog & News</h1>
        <p className="text-secondary-foreground/70 mt-2">Latest updates, tips, and stories from Bhavishya Education</p>
      </div>
    </div>
          ...
          {blogs.map((b, i) => (
            <article key={i} className="bg-card rounded-2xl border border-border overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 group">
              ...
              <div className="p-5 space-y-3">
                ...
                <h3 className="font-heading text-base font-semibold text-foreground leading-snug">{b.title}</h3>
                <p className="text-sm text-muted-foreground">{b.excerpt}</p>
                <a href="#" className="inline-flex items-center gap-1 text-sm font-semibold text-primary hover:underline">Read More <ArrowRight size={14} /></a>
              </div>
            </article>
          ))}
        </div>
